...
CreateStateIntersectFile <- function(Sample_file= Sample_df,
                                     ...
                                     Group_states=T, ## group states from 100 to 14. F if you want 100 or are providing your own model
  ...
  ### Catch for E2G style Element- prefix or standard chr start end
  if( length(colnames(e2g.table)[colnames(e2g.table) %in% c("ElementChr","ElementStart","ElementEnd")])>0 ){
    ...
    tmp.e2g.table$end <- tmp.e2g.table$ElementEnd3
  } else {
    tmp.e2g.table <- e2g.table[,c("chr","start","end")]
    tmp.e2g.table$Region <- c(1:nrow(tmp.e2g.table))
    
  }
  
  
  ###Check if data is ready with chr start end or stop function
  if( length(colnames(e2g.table)[colnames(e2g.table) %in% c("chr","start","end")])<3 ){
    print("Input Sample_file lacks chr start end format, ending")
    return(NULL)
  }
  ...
  reference.table$Class[is.na(reference.table$Class)]<- "Other"
    ...
    ### If you are not grouping states
    reference.table <- cbind.data.frame(state=sort(unique(chromhmm.table$state)), Class=sort(unique(chromhmm.table$state) ) )
  }
  ...
  final.e2g.table$Region<- NULL
    ...
    write.table(final.e2g.table, gzfile(paste0(outdir,ExpName, "E2G_table.with.ChromHMM.state.overlap.features.txt.gz")),
